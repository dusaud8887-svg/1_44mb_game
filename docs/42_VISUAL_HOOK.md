...
정본 키아트: [keyart_last_live.px](../assets/px/keyart_last_live.px) (생성기 `tools/px_keyart.py`, 렌더 `python3 tools/px_render.py assets/px/keyart_last_live.px --scale 3`). 192×108 저해상 원본 → 정수배 업스케일로 각 표면에 대응.
...
- **모션 파생**: 정지 키아트에서 커서 깜빡임·글로우 맥동·조각 표류·눈 1px 슬라이드만 애니메이트하면 소셜용 루프 GIF가 된다(각 4프레임 상한, 40 §1 모션 문법 재사용).
