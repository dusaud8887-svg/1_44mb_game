...
if (isOpen)
{
    ...
    for (var i = 0; i < array_length(output); i++)
    {
        ...
        if (string_char_at(outputStr, 1) == ">")
        {
            ...
            draw_text_scribble(shellOriginX + consolePaddingH, yOffset, prompt);
            draw_text_scribble_ext(shellOriginX + promptXOffset, yOffset, string_delete(outputStr, 1, 1), -1, visibleWidth - promptXOffset);
        }
        else
        {
            draw_set_color(fontColor);
            draw_text_scribble_ext(shellOriginX + promptXOffset, yOffset, outputStr, -1, visibleWidth - promptXOffset);
        }
    ...
    draw_text_scribble(shellOriginX + consolePaddingH, yOffset, prompt);
    draw_set_color(fontColor);
    draw_text_scribble(shellOriginX + promptXOffset, yOffset, consoleString);
    ...
    else
    {
        ...
        draw_set_color(promptColor);
        draw_text_scribble(cursorPosX, yOffset, string_copy(consoleString, cursorPos, 1));
    }
    if (array_length(inputArray) > 0)
    {
        ...
        if (data != undefined)
        {
            ...
            for (var i = spaceCount; i < array_length(variable_struct_get(data, "arguments")); i++)
            {
                args += " ";
        ...
        draw_set_color(fontColorSecondary);
        draw_text_scribble(shellOriginX + promptXOffset + string_width(consoleString), yOffset, suggestion);
    }
        ...
        if (enableAutocomplete && autocompleteMaxLines > 0)
        {
            ...
            for (var i = 0; i < array_length(filteredSuggestions); i++)
            {
                if (i < suggestionsAmount)
                {
                    ...
                    draw_text_scribble(x1 + autocompletePadding, y1 + (i * emHeight) + autocompletePadding, filteredSuggestions[i + autocompleteScrollPosition]);
                }
            }
        }
    }
