function glr_draw_gamma(arg0)
{
    ...
    var sur_w = surface_get_width(arg0);
    var sur_h = surface_get_height(arg0);
    ...
    if (shadowarea_sz > 0 || (global.GLR_DIRECTIONAL_ENABLED && global.GLR_DIRECTIONAL_STRENGTH > 0))
    {
        ...
        var ss = (1 - global.GLR_DIRECTIONAL_STRENGTH) * 255;
        var col = make_colour_rgb(min(255, global.GLR_AMBIENT_R * ss), min(255, global.GLR_AMBIENT_G * ss), min(255, global.GLR_AMBIENT_B * ss));
    ...
    for (var i = 0; i < ds_list_size(global.GLR_LIGHT_LIST); i++)
    {
        ...
        shader_set_uniform_f(global.GLR_UNIF_LIGHT_INTENSITY, ds_list_find_value(l_id, UnknownEnum.Value_13));
    ...
    for (var i = 0; i < ds_list_size(global.GLR_LIGHT_LIST_SIMPLE); i++)
    {
        ...
        shader_set_uniform_f(global.GLR_UNIF_LIGHT_INTENSITY_SIMPLE, ds_list_find_value(l_id, UnknownEnum.Value_13));
    ...
    if (surface_exists(global.GLR_APP_SURFACE_COPY))
    {
        if (surface_get_width(global.GLR_APP_SURFACE_COPY) != surface_get_width(arg0) || surface_get_height(global.GLR_APP_SURFACE_COPY) != surface_get_height(arg0))
        {
            surface_free(global.GLR_APP_SURFACE_COPY);
            global.GLR_APP_SURFACE_COPY = surface_create(surface_get_width(arg0), surface_get_height(arg0));
        }
    }
    else
    {
    ...
    surface_copy(global.GLR_APP_SURFACE_COPY, 0, 0, arg0);
    if (global.GLR_BLUR_ENABLED)
    {
        ...
        gpu_set_texfilter_ext(global.GLR_SAMPLER_APP_BLUR, false);
        texture_set_stage(global.GLR_SAMPLER_APP_BLUR, surface_get_texture(global.GLR_APP_SURFACE_COPY));
    ...
    else
    {
        ...
        if (global.GLR_FXAA_ENABLED)
        {
            ...
            gpu_set_texfilter_ext(global.GLR_SAMPLER_APP_FXAA, false);
            texture_set_stage(global.GLR_SAMPLER_APP_FXAA, surface_get_texture(global.GLR_APP_SURFACE_COPY));
        }
        else
        {
            shader_set(global.GLR_OS_GAMMA_SHADER);
            gpu_set_texfilter_ext(global.GLR_SAMPLER_APP, false);
            texture_set_stage(global.GLR_SAMPLER_APP, surface_get_texture(global.GLR_APP_SURFACE_COPY));
        }
        var offsetX = 0;
        var offsetY = 0;
        var sizeX = 0;
        ...
        if (v_x0 < 0)
        {
            offsetX = floor(v_x0) - v_x0;
            sizeX = -offsetX;
        }
        else
        {
            sizeX = v_x0 - floor(v_x0);
        }
        if (v_y0 < 0)
        {
            offsetY = floor(v_y0) - v_y0;
            sizeY = -offsetY;
        }
        else
        {
            sizeY = v_y0 - floor(v_y0);
        }
