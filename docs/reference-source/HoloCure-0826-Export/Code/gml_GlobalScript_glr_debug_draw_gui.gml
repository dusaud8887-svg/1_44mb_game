function glr_debug_draw_gui()
{
    ...
    {
        str = "[ GLARE ]   #" + string(fps) + " fps#" + "Quality: " + string(quality * 100) + "%#" + "ViewSize: " + string(global.GLR_WIDTH) + "x" + string(global.GLR_HEIGHT) + "#" + string(ds_list_size(global.GLR_MESH_DYN_LIST)) + " dynamic meshes#" + string(ds_list_size(global.GLR_MESH_STC_LIST)) + " is_static meshes#" + string(ds_list_size(global.GLR_LIGHT_LIST)) + " lights#" + string(ds_list_size(global.GLR_LIGHT_LIST_SIMPLE)) + " simple lights#" + "Fxaa: " + fxaa_str + "#" + "Directional: " + directional_str + "#" + "Occlusion: " + occlusion_str + "#" + "Blur: " + blur_str;
        sh = 200;
    }
    else
    {
        str = "[ GLARE ]           #" + string(fps) + " fps";
        sh = 50;
    }
    ...
    draw_text(15, 15, string_hash_to_newline(str));
    var icon;
    if (global.GLR_DEBUG_ACTIVE)
    {
        icon = "_";
    ...
    draw_set_alpha(1);
    draw_set_color(c_black);
    draw_text(px + 5, py + 5, string_hash_to_newline(icon));
    if (global.GLR_DEBUG_ACTIVE)
    {
        var padding = 5;
        var spacing = 8;
        ...
        for (var j = 0; j < sz; j++)
        {
            var text = ds_list_find_value(l, j);
            w = (7 * string_length(text)) + (padding * 2);
            clicked = false;
            for (var i = 0; i < 5; i++)
            {
                if (device_mouse_check_button_pressed(i, mb_left))
            ...
            draw_set_alpha(1);
            draw_set_color(c_black);
            draw_text(px + padding, py + padding, string_hash_to_newline(text));
            px += (w + spacing);
            if (clicked)
            {
                switch (text)
