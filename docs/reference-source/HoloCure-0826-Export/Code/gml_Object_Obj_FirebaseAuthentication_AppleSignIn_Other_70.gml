...
switch (ds_map_find_value(async_load, "id"))
{
    ...
    case applesignin_signin_response:
        var appleSignInResponse = ds_map_find_value(async_load, "response_json");
        ...
        if (ds_map_find_value(jsonMap, "success"))
        {
            show_debug_message("Apple Sign In Succeeded");
            var identityToken = ds_map_find_value(jsonMap, "identityToken");
            token = identityToken;
            event_user(0);
        }
        else
