...
if (inView)
{
    ...
    depth = -y;
    draw_sprite_ext(sprite_index, broken, x + ((hitShake > 0) * shakeDisplacement), y - 4, 1, -0.7, 0, c_black, 0.3);
    draw_sprite_ext(sprite_index, broken, x + ((hitShake > 0) * shakeDisplacement), y, image_xscale, image_yscale, image_angle, c_white, image_alpha);
    gpu_set_blendmode(bm_normal);
    shader_reset();
    if (instance_exists(obj_PlayerManager) && obj_PlayerManager.showDamageData)
    {
        if (timeStartedAttacking > -1 && global.debug)
        {
            ...
            draw_text(x, y + 20, totalDamageTaken);
        }
    }
}
