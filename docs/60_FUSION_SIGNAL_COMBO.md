...
### 6-2. 구현됨 — 학파별 융합 유틸리티 3종 (id 12/13/14)
각 학파 정체성을 융합 루프의 한 축에 직접 건 유틸리티 프로그램. 킹덤 풀 10→13, 카드 15→18종.

| 카드 | 학파(태그) | 킹덤 역할 | 효과(발동) | 융합 축 |
|---|---|---|---|---|
| **신호 자기장 MAGNET** (자장) | 확산 NETWORK | payload | 이번 구절 자석 반경 +`MAGNET_BONUS` | 신호 **회수**(경제) |
| **신호 정지 FREEZE** (정지) | 내성 SAFE | safe | 피격 `FREEZE_CHARGES`회 연쇄 유지(보험 적립) | 연쇄 **보존**(리스크 완화) |
| **연산 가속 OVERCLOCK** (가속) | 연사 REPLAY | payload | 반송파 과부하 `OVERCLOCK_TICKS` + 연쇄 창 갱신 | 연쇄 **지속**+화력 |

- NOA 복제 시 각각 습격 증폭(적 2체 스폰).
- `test_signal_combo`에 3종 회귀 추가(자석 반경 증가·연쇄 보험 소비/붕괴·과부하+창 갱신·태그·킹덤 유효성). 킹덤 풀 확대로 SIM 분포가 미세 이동했으나(§4) 불변식(THREE_WAY 최단 생존·CLEAN_SIGNAL 최장) 유지, 기본 전투(base surv479) 불변.

### 6-3. 남은 후보 (다음 패스)
| 후보 카드 | 학파 | 효과 초안 | 비고 |
|---|---|---|---|
| **환류 FEEDBACK** | 엔진 | 신호 소모 → 편성 +1 즉시 | 타이밍(지난 구절 은행분 사용) 설계 필요 |
