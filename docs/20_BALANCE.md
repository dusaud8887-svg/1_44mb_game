...
| `COST_AMP` / `AMP_COMBO_JUMP` | 3 / 6 | 공명 증폭 킹덤 카드: 발동 시 연쇄 급증(1.5티어)+구절 조각 가치 +1 |
| `COST_SCAN` | 2 | 회선 스캔(엔진 캔트립): 편성 cue 순증0 + 손패 순환 → 터미널 충돌 해소(드로우 축) |
| `COST_MAGNET`·`MAGNET_BONUS` | 3·40 | 자기장(확산/payload): 구절 자석 반경↑ → 신호 회수 |
| `COST_FREEZE`·`FREEZE_CHARGES` | 3·1 | 정지(내성/safe): 피격 연쇄 유지 보험 |
| `COST_OVERCLOCK`·`OVERCLOCK_TICKS/_COMBO_WINDOW` | 4·120/240 | 가속(연사/payload): 반송파 과부하 + 연쇄 창 갱신 |
| `SPECIAL_*` (100/7/…) | 필살기 충전·학파별 위력 | 처치 충전·`J` 발동. SIM 봇 미사용 → SIM 불변 |
| `ELITE_*` (6/5/5/40) | 등장 턴/HP배수/조각/충전 | 6구절부터 탱키 슈터 1체. SAFE 방어 가치↑ → 위험/생존 불변식 강화 |
