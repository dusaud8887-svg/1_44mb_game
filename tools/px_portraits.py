...
def portrait_seek():
    ...
    for y in range(26, 42):                     # 볼 양옆을 어둠이 감싸 얼굴을 좁게 + 부드러운 턱
        for x in range(11, 37):
            if g[y][x] == "3" and abs(x - 23) > max(3, 9 - (y - 26) // 2):
                g[y][x] = "1"
    # 호박 반개 눈(강렬): 낮은 꺼풀. V2(06 §2) 초상=인간형 아바타·두 눈이되 한쪽은 가림
    for ex in (16, 25):
    ...
    # 한쪽 눈 가림(완전히 읽히지 않는 한쪽 눈): 왼쪽을 후드 사선으로 덮음
    for x in range(11, 24):
        for y in range(24, 34 - (x - 11)):
            if 0 <= y < 48:
                g[y][x] = "1"
    px(g, 15, 30, "2"); px(g, 16, 31, "2")      # 가린 눈에서 새어나오는 호박 1px
    hline(g, 23, 27, 34, "1")                   # 입: 작고 평평
    px(g, 27, 35, "3")                          # 송곳니 1px(입꼬리, 06 §2 브리지)
...
def portrait_format():
    # V2 재설계(06 §3, 40 §4): FORMAT/0 → 노아(NØA). "완벽한 관객" 최종보스의 FIRST
    # MODERATOR 초상 — 아름답고 안정, 위험은 그림자·베일에만. 폼 문법=완전 대칭·수직선·
    # 이중 테두리. 반개 눈=경청(냉소 아님), 눈동자 하이라이트=프로필 점, 댓글 베일(수직
    # 채팅창), 한쪽 흰 장갑, 프로필 아이콘 환, 아주 작은 예의상 미소. (파일명은 코드 V1
    # 슬롯 호환 위해 format 유지 — 자산은 NØA, ADR-0003/0008.)
    g = blank()
    # 어깨 + 세운 하이칼라(다크) — 대칭
    ellipse(g, 24, 55, 20, 12, "1")
    rect(g, 15, 38, 33, 48, "1")
    # 긴 히메 뒷머리(수직 커튼) + 정수리 → 얼굴 (완전 대칭)
    rect(g, 9, 13, 39, 47, "1")
    ellipse(g, 24, 21, 16, 15, "1")
    ellipse(g, 24, 25, 11, 13, "3")            # 얼굴(광): 세로 오벌, 대칭
    for y in range(32, 40):                     # 부드러운 턱(대칭)
        halfw = max(4, 10 - (y - 31))
        for x in range(48):
            if g[y][x] == "3" and abs(x - 24) > halfw:
                g[y][x] = "1"
    # 히메 앞머리(일자 블런트 뱅) + 사이드록(양쪽 동일=대칭)
    for x in range(12, 37):
        for y in range(6, 18):
            if g[y][x] == "3":
                g[y][x] = "1"
    rect(g, 10, 16, 14, 44, "1"); rect(g, 34, 16, 38, 44, "1")
    # 반개 눈=경청(부드러움, 냉소 아님): 낮은 꺼풀 + 자홍 홍채 + 눈동자 프로필 점 하이라이트
    for ex in (16, 26):
        hline(g, ex, ex + 5, 23, "1")          # 위 반개 꺼풀
        rect(g, ex, 24, ex + 5, 26, "2")       # 자홍 홍채(넓게=경청 눈)
        vline(g, ex + 2, 25, 26, "1")          # 동공(부드럽게)
        px(g, ex + 1, 24, "3"); px(g, ex + 4, 25, "3")  # 눈동자 프로필 점 하이라이트(4개)
        hline(g, ex + 1, ex + 4, 27, "1")      # 아래 라인
    # 눈썹: 곧고 수평·대칭(에코의 비대칭과 반대)
    hline(g, 16, 20, 21, "1"); hline(g, 27, 31, 21, "1")
    # 코 점 + 아주 작은 예의상 미소(중앙 대칭, 양끝 살짝 올림)
    px(g, 24, 29, "1")
    hline(g, 22, 26, 31, "1"); px(g, 21, 30, "1"); px(g, 27, 30, "1")
    # 댓글 베일: 하이칼라에서 수직으로 내려오는 채팅창 줄(자홍 세그먼트) — 대칭
    for cy in range(39, 47, 2):
        rect(g, 18, cy, 22, cy, "2")           # 왼 채팅줄
        rect(g, 26, cy, 30, cy, "2")           # 오른 채팅줄
        px(g, 17, cy, "1"); px(g, 24, cy, "1"); px(g, 31, cy, "1")  # 이중 테두리 칸막이
    frame_none = None  # (이중 테두리 프레임)
    hline(g, 16, 32, 38, "2"); hline(g, 16, 32, 47, "2")
    vline(g, 16, 38, 47, "2"); vline(g, 32, 38, 47, "2")
    vline(g, 14, 38, 47, "2"); vline(g, 34, 38, 47, "2")  # 이중 테두리(수직선·대칭)
    # 프로필 아이콘 환: 머리 위 작은 자홍 점 4~8개(대칭 아치)
    for a in (35, 60, 90, 120, 145):
        rad = math.radians(a)
        x = int(round(24 + 15 * math.cos(rad)))
        y = int(round(9 - 8 * math.sin(rad)))
        if 0 <= x < 48 and 0 <= y < 48:
            px(g, x, y, "2")
    # 한쪽 흰 장갑(only one, 06 §3): 베일 오른쪽을 반듯하게 정돈하는 장갑 손 — 베일 위에
    rect(g, 28, 37, 33, 41, "3")               # 손등(장갑)
    px(g, 30, 36, "3"); px(g, 32, 36, "3")     # 손가락(정돈 중)
    px(g, 34, 39, "3")                         # 소매 끝
    hline(g, 29, 32, 42, "1")                  # 장갑 아래 그림자
    save("assets/px/portrait_format_48.px",
         g, "1=120e1a 2=ef4f9e 3=f2ebdd",
         "portrait_format_48 - 노아(NØA) FIRST MODERATOR 초상 (V2, 자산은 NØA)")
