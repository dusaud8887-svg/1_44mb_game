...
def portrait_echo():
    ...
    for x in range(21, 30):
        ...
        px(g, x, min(yy, 34), "1")
    # 볼 터치(청록 디더 = 모니터 광)
    dither(g, 15, 28, 18, 29, "2", base="3")
    dither(g, 32, 28, 35, 29, "2", base="3")
...
def portrait_seek():
    ...
    px(g, 29, 43, "3")
    # 리본 케이블: 오른쪽 분절 3개
    rect(g, 42, 30, 45, 31, "2")
    rect(g, 44, 34, 47, 35, "2")
    rect(g, 43, 38, 45, 39, "2")
    # 배 데이터 밴드(호박 디더 1줄)
...
def portrait_format():
    ...
    # 헤어 엣지 스파클(각 바깥 1열만 체커)
    for y in range(6, 46, 2):
        px(g, 12, y, "2")
    for y in range(9, 40, 2):
        px(g, 35, y, "2")
    # 얼굴(광): 좁은 세로 타원
    ellipse(g, 24, 24, 8, 11, "3")
    # 앞머리: 히메컷 일직선 + 비대칭(왼쪽 한 단 더 김)
    rect(g, 13, 4, 34, 13, "1")
    rect(g, 16, 14, 20, 15, "1")
